...
def write_main_page(streams):
    ...
    main_text = '<br>'.join(
        ...
        '',''
        '<br>'.join([f'<div style="align-items: center; display: flex;"><img width="60px" height="60px" style="border: 1px solid black;" src="{v["logo"]}"</img><audio controls style="margin-left:10px; width:50px;" src="{v["streamLink"]}"></audio> <div> <a target="_blank" href="{v['mainLink']}">{k}</a><br>{to_one_line(v)}<br>{v["location"]}<br>{v["status"]}</div></div>' for k,v in streams.items()]),
        '',
