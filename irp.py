...
def write_main_page(streams):
    ...
    main_text = '<br>'.join(
        ['<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Internet Radio Protocol</title><link rel="icon" href="assets/icon-dark.ico" type="image/x-icon"><link rel="shortcut icon" href="assets/icon-dark.ico" type="image/x-icon"><link rel="apple-touch-icon" href="assets/icon-dark.png"></head><body style="font-family:Andale Mono; padding:10vw; padding-top:10px;"><div style="display:flex; justify-content:center"><img src="assets/circleblack.gif" alt="Loading" height="250px" width="250px" style="margin-left:-25px;"></div>',
        ...
        '',
        'If you have any questions, comments, or radio station addition suggestions, please email <a href="mailto:bmo@scudhouse.com">bmo@scudhouse.com</a>.',
        '', '',
        '<br>'.join([f'''<div id="{k}" style="align-items: center; display: flex;">
            ...
            <div style="font-size:10pt;"> 
                <a target="_blank" href="{v['mainLink']}">{k}</a><br>Now Playing: {v.get('oneLiner')}<br>Track ID: {v.get('shazamGuess')}<br>Location: {v["location"]}<br>Status: {v["status"]}<br>Last Updated: <span class="last-updated" data-utc="{v['lastUpdated']}">{v['lastUpdated']}</span><br>
