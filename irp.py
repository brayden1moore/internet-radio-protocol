...
def write_main_page(streams):
    ...
    main_text = '<br>'.join(
        ...
        "<script>function toggleAudio(id){document.querySelectorAll('audio').forEach(a=>{a.pause();a.src='';a.parentElement.parentElement.style.backgroundColor='white'});let a=document.getElementById(`${id}-audio`),d=document.getElementById(id);if(d.style.backgroundColor!=='yellow'){if(!a.src)a.src=a.dataset.src;a.play();d.style.backgroundColor='yellow';}}</script>",
