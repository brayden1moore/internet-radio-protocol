...
def write_main_page(streams):
    ...
    main_text = '<br>'.join(
        ...
        '',''
        '<br>'.join([f'<div style="align-items: center; display: flex;"><img width="70px" height="70px" style="margin-left:10px; margin-right:10px; border: 1px solid black;" src="{v["logo"]}"</img> <div> <a target="_blank" href="{v['mainLink']}">{k}</a><br>{to_one_line(v)}<br>{v["location"]}<br>{v["status"]}<audio controls style="width:40px;" src="{v["streamLink"]}"></audio></div></div>' for k,v in streams.items()]),
        '',
