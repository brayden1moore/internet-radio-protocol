...
with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
    results = executor.map(fetch_data, list_resp), total=len(list_resp)
...
with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
    results = executor.map(check_live, untracked_live_stations), total=len(untracked_live_stations)
